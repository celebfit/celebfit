...
def get_cached_module_file(
    ...
    if os.path.isfile(module_file_or_url):
        ...
        try:
            ...
            resolved_module_file = os.path.join(cache_dir, pretrained_model_name_or_path + ".py")
            if not os.path.exists(resolved_module_file) or force_download:
                r = requests.get(github_url, proxies=proxies)
                r.raise_for_status()
                with open(resolved_module_file, "wb") as f:
                    f.write(r.content)

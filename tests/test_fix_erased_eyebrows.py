...
output_dir = os.path.join(root_path, "tests/data/eyebrow_tests/fix_verification")
...
def main():
    ...
    for img_file in test_files:
        ...
        # 2. No Inpaint (Original Eyebrows)
        input_no_inpaint = image_512.copy()
        
        # 3. Gaussian Blur (Blur Eyebrows)
        blurred_eyebrows = cv2.GaussianBlur(image_512, (25, 25), 0)
        input_blur = (image_512 * (1.0 - mask_3ch_smooth) + blurred_eyebrows * mask_3ch_smooth).astype(np.uint8)
        
        methods = [
            {"name": "Telea_Inpaint", "img": input_telea},
            {"name": "No_Inpaint", "img": input_no_inpaint},
            {"name": "Gaussian_Blur", "img": input_blur}
        ]
        
        rows = []
        
        # Let's generate for each method and celebrity
        for method in methods:
            method_name = method["name"]
            input_img_512 = method["img"]
            image_pil = Image.fromarray(cv2.cvtColor(input_img_512, cv2.COLOR_BGR2RGB))
            pipe_mask_pil = Image.new("RGB", (512, 512), "white")
            
            # Use Canny guide on the input image
            control_image_pil = Image.fromarray(np.zeros((512, 512, 3), dtype=np.uint8)) # Keep Canny blank or use canny guide from original image
            img_canny = cv2.Canny(image_512, 100, 200)
            img_canny = img_canny[:, :, None]
            img_canny = np.concatenate([img_canny, img_canny, img_canny], axis=2)
            control_image_pil = Image.fromarray(img_canny)
            
            panels = [cv2.resize(cv2.cvtColor(input_img_512, cv2.COLOR_BGR2RGB), (256, 256))]
            cv2.putText(panels[0], method_name, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            ...
            for case in comparison_cases:
                ...
                print(f"  - Method {method_name} | Celebrity {display_name}...")
        ...
        grid = np.vstack(rows)
        grid_path = os.path.join(output_dir, f"verification_{img_basename}.png")
        Image.fromarray(grid).save(grid_path)
        print(f"Saved verification grid to: {grid_path}")
